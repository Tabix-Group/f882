...
const F88AssessmentPage = () => {
    ...
    const checkExistingAssessment = (0, react_1.useCallback)(() => __awaiter(void 0, void 0, void 0, function* () {
        try {
            const response = yield fetch(`http://localhost:4000/api/training/assessment/${user === null || user === void 0 ? void 0 : user.id}`);
            if (response.ok) {
    ...
    const handleSubmit = (e) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        setIsLoading(true);
        try {
            const response = yield fetch('http://localhost:4000/api/training/assessment', {
                method: 'POST',
                headers: {
    ...
    const renderQuestion1 = () => (<div className="space-y-6">
        ...
        ].map((option) => (<label key={option.value} className={`block p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 ${formData.question1 === option.value
                ? 'border-blue-500 bg-blue-500/20 text-white'
                : 'border-white/20 bg-white/5 text-gray-300 hover:border-white/40'}`}>
                        <input type="radio" name="question1" value={option.value} checked={formData.question1 === option.value} onChange={(e) => handleInputChange('question1', parseInt(e.target.value))} className="mr-3"/>
                        {option.text}
                    </label>))}
            </div>
        </div>);
    const renderQuestion2 = () => (<div className="space-y-6">
            <div className="text-center mb-8">
        ...
        ].map((option) => (<label key={option.value} className={`block p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 ${formData.question2 === option.value
                ? 'border-blue-500 bg-blue-500/20 text-white'
                : 'border-white/20 bg-white/5 text-gray-300 hover:border-white/40'}`}>
                        <input type="radio" name="question2" value={option.value} checked={formData.question2 === option.value} onChange={(e) => handleInputChange('question2', parseInt(e.target.value))} className="mr-3"/>
                        {option.text}
                    </label>))}
            </div>
        </div>);
    const renderQuestion3 = () => (<div className="space-y-6">
            <div className="text-center mb-8">
        ...
        ].map((option) => (<label key={option.value} className={`block p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 ${formData.question3 === option.value
                ? 'border-blue-500 bg-blue-500/20 text-white'
                : 'border-white/20 bg-white/5 text-gray-300 hover:border-white/40'}`}>
                        <input type="radio" name="question3" value={option.value} checked={formData.question3 === option.value} onChange={(e) => handleInputChange('question3', parseInt(e.target.value))} className="mr-3"/>
                        {option.text}
                    </label>))}
            </div>
        </div>);
    const renderRestDaySelection = () => (<div className="space-y-6">
            <div className="text-center mb-8">
        ...
        ].map((day) => (<label key={day.value} className={`block p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 text-center ${formData.restDay === day.value
                ? 'border-blue-500 bg-blue-500/20 text-white'
                : 'border-white/20 bg-white/5 text-gray-300 hover:border-white/40'}`}>
                        <input type="radio" name="restDay" value={day.value} checked={formData.restDay === day.value} onChange={(e) => handleInputChange('restDay', e.target.value)} className="sr-only"/>
                        {day.label}
                    </label>))}
            </div>
        </div>);
            ...
            <div className="relative z-10 min-h-screen flex flex-col">
                    ...
                    <div className="max-w-4xl mx-auto flex items-center gap-4">
                        ...
                        <div className="ml-auto flex items-center gap-4">
                            <div className="flex gap-1">
                                {[1, 2, 3, 4, 5].map((step) => (<div key={step} className={`w-3 h-3 rounded-full ${step <= currentStep ? 'bg-blue-500' : 'bg-white/20'}`}/>))}
                            </div>
                ...
                <div className="flex-1 flex items-center justify-center px-4 py-8">
                    <div className="max-w-2xl w-full">
                        {currentStep === 1 && (<div className="text-center mb-8">
                        ...
                        <div className="flex justify-between mt-8 gap-4">
                            {currentStep > 1 && (<button onClick={prevStep} className="px-6 py-3 bg-white/10 hover:bg-white/20 text-white font-medium rounded-xl transition-all duration-200">
                                    Anterior
                                </button>)}

                            <div className="flex-1"/>

                            {currentStep < 5 ? (<button onClick={nextStep} disabled={!isStepValid()} className="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed text-white font-medium rounded-xl transition-all duration-200">
                                    Siguiente
                                </button>) : (<button onClick={handleSubmit} disabled={isLoading} className="px-8 py-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed text-white font-medium rounded-xl transition-all duration-200">
...
exports.default = F88AssessmentPage;
