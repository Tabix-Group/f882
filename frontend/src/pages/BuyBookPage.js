...
const BuyBookPage = () => {
    ...
    const handleToggle = (id) => {
        setSelected((prev) => (Object.assign(Object.assign({}, prev), { [id]: !prev[id] })));
    };
    const handleProceed = () => {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 grid gap-6">
            {PRODUCTS.map((product) => (<article key={product.id} className={`group bg-white/4 rounded-3xl p-6 md:p-8 shadow-md transition-transform duration-200 ${selected[product.id] ? 'ring-2 ring-blue-500 scale-102' : 'hover:-translate-y-1 hover:shadow-xl'}`} onClick={() => handleToggle(product.id)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            handleToggle(product.id);
        } }} role="button" tabIndex={0} aria-pressed={!!selected[product.id]}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-4 flex items-center gap-4">
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input aria-label={`Seleccionar ${product.title}`} type="checkbox" checked={!!selected[product.id]} onChange={() => handleToggle(product.id)} className="accent-blue-500 w-5 h-5 rounded" onClick={(e) => e.stopPropagation()}/>
                        <span className="text-base">Seleccionar</span>
                      </label>

                      {product.links.length > 0 && (<div className="flex flex-col">
                          {product.links.map((link, idx) => (<a key={idx} href={link.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 underline hover:text-blue-200 text-sm">{link.label}</a>))}
                        </div>)}
                    </div>
                  </div>
                </div>
                ...
                {product.id === 'book-mentor' && (<div className="mt-4 inline-block bg-gradient-to-r from-yellow-500 to-orange-400 text-black font-semibold px-3 py-1 rounded-full text-sm">Más completo</div>)}
              </article>))}
          </div>
          ...
          <aside className="md:col-span-1 sticky top-24 self-start">
            <div className="bg-white/4 rounded-3xl p-6 shadow-lg border border-white/8">
              ...
              <ul className="mt-4 divide-y divide-white/6">
                {PRODUCTS.filter((p) => selected[p.id]).length === 0 && (<li className="text-gray-300 py-4">No has seleccionado productos</li>)}
                {PRODUCTS.filter((p) => selected[p.id]).map((p) => (<li key={p.id} className="py-4 flex items-center justify-between">
              ...
              <button onClick={handleProceed} disabled={total === 0} aria-disabled={total === 0} className={`mt-6 w-full py-3 rounded-full font-bold text-lg transition-all duration-200 ${total === 0 ? 'bg-gray-600 cursor-not-allowed text-gray-300' : 'bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600'}`}>
                Proceder al Pago
              </button>
            </div>
          </aside>
