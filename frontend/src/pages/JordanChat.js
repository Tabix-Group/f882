...
const JordanChat = () => {
    ...
    (0, react_1.useEffect)(() => {
        const loadChatHistory = () => __awaiter(void 0, void 0, void 0, function* () {
            if (!user) {
                navigate('/login');
                return;
            }
...
¡Estoy emocionado de ser parte de tu viaje de transformación!`,
            ...
            try {
                ...
                const sessionsResponse = yield fetch(`http://localhost:4000/sessions/${user.id}`);
                const sessionsData = yield sessionsResponse.json();
                let currentSessionId = null;
                if (sessionsData.sessions && sessionsData.sessions.length > 0) {
                    // Usar la sesión más reciente
                    currentSessionId = sessionsData.sessions[0].id;
                    // Cargar mensajes de la sesión
                    const messagesResponse = yield fetch(`http://localhost:4000/messages/${currentSessionId}`);
                    ...
                    const newSessionResponse = yield fetch('http://localhost:4000/session', {
                        method: 'POST',
                        headers: {
                    ...
                    const newSessionData = yield newSessionResponse.json();
                ...
                if (savedMessages) {
                    const parsedMessages = JSON.parse(savedMessages).map((msg) => (Object.assign(Object.assign({}, msg), { timestamp: new Date(msg.timestamp) })));
    ...
    const sendMessage = (e) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        setIsLoading(true);
        try {
            const response = yield fetch('http://localhost:4000/send', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
...
¿Hay algo específico en lo que necesites ayuda o consejo?`,
    ...
    const downloadChat = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            let chatData = null;
            if (sessionId) {
                const response = yield fetch(`http://localhost:4000/messages/${sessionId}`);
                const data = yield response.json();
                if (data.messages) {
                    chatData = {
            ...
            const exportFileDefaultName = `chat_jordan_${user === null || user === void 0 ? void 0 : user.name}_${new Date().toISOString().split('T')[0]}.json`;
    ...
    const clearChat = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            if (sessionId) {
                ...
                yield fetch(`http://localhost:4000/session/${sessionId}`, {
                    method: 'DELETE'
                });
            }
            ...
            localStorage.removeItem(`chat_history_${user === null || user === void 0 ? void 0 : user.id}`);
        }
        catch (error) {
    ...
    return (<div className="bg-gradient-to-br from-black via-neutral-900 to-black text-white min-h-screen flex flex-col relative overflow-hidden">
            {/* Background Effects */}
            ...
            <div className="relative z-10 flex flex-col min-h-screen">
                ...
                <div className="bg-neutral-900/80 backdrop-blur-md border-b border-white/10 px-3 sm:px-4 py-3 sm:py-4 sticky top-0 z-10">
                    <div className="max-w-4xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
                        <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
                            <button onClick={() => navigate('/dashboard')} className="p-2 hover:bg-white/10 rounded-lg transition-colors duration-200 flex-shrink-0">
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                            <button onClick={downloadChat} disabled={messages.length === 0} className="p-2 hover:bg-white/10 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed" title="Descargar chat">
                                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                </svg>
                            </button>
                            <button onClick={clearChat} disabled={messages.length === 0} className="p-2 hover:bg-red-500/20 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed" title="Limpiar chat">
                                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-4 sm:py-6">
                    <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
                        {messages.length === 0 && !isLoading && (<div className="text-center py-8 sm:py-12">
                        ...
                        {messages.map((message) => (<div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] sm:max-w-xs lg:max-w-md px-3 sm:px-4 py-2 sm:py-3 rounded-2xl shadow-lg ${message.role === 'user'
                    ...
                    <div className="max-w-4xl mx-auto">
                        <form onSubmit={sendMessage} className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                            <div className="flex-1 relative">
                                <input type="text" value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} placeholder="Escribe tu mensaje a Jordan..." className="w-full px-4 py-3 sm:py-4 bg-white/5 border border-white/20 rounded-2xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base resize-none" disabled={isLoading} maxLength={1000}/>
                                <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-gray-500">
                                    {inputMessage.length}/1000
                                </div>
                            </div>
                            <button type="submit" disabled={!inputMessage.trim() || isLoading} className="px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed text-white font-medium rounded-2xl transition-all duration-200 hover:scale-105 shadow-lg hover:shadow-blue-500/25 flex items-center justify-center gap-2 min-w-[100px] sm:min-w-[120px]">
                                {isLoading ? (<>
